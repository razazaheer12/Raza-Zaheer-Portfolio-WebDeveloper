...
const Footer = () => {
  ...
  return (
    <footer className="relative bg-[#080B14] text-gray-300 overflow-hidden border-t border-white/5">
      
      {/* Ambient Background Glows */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -left-32 bottom-0 h-[250px] w-[250px] rounded-full bg-purple-600/10 blur-[120px]" />
        <div className="absolute right-[-80px] bottom-0 h-[220px] w-[220px] rounded-full bg-blue-600/10 blur-[120px]" />
      </div>

      <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 pt-8 pb-8">
        
        {/* Top Section - Circular Back to Top Button */}
        <div className="pb-8 border-b border-white/10 flex justify-start">
          <motion.button
            onClick={scrollToTop}
            whileHover={{ scale: 1.08 }}
            whileTap={{ scale: 0.92 }}
            aria-label="Back to top"
            className="w-12 h-12 rounded-full border border-white/20 bg-transparent flex items-center justify-center text-gray-300 hover:border-purple-500 hover:text-purple-400 hover:bg-purple-500/10 hover:shadow-[0_0_20px_rgba(168,85,247,0.4)] transition-all duration-300"
          >
            <ChevronUp size={22} />
          </motion.button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 py-10">
          ...
          {/* Left Column: MRZ Logo & Description */}
              ...
              {socialLinks.map((social) => {
                ...
                return (
                  <motion.a
                    ...
                    whileHover={{ scale: 1.08 }}
                    whileTap={{ scale: 0.95 }}
                    className="w-10 h-10 rounded-xl border border-white/10 bg-white/[0.03] flex items-center justify-center text-gray-400 hover:border-purple-500 hover:text-purple-400 hover:bg-purple-500/10 transition-all duration-300"
                  >
                    <Icon size={16} />
                  </motion.a>
                );
